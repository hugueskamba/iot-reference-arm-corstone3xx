...
static bool ml_lock()
{
    ...
    BaseType_t ret = xSemaphoreTake( xMlMutex, portMAX_DELAY );
    if (ret != pdTRUE) {
...
static bool ml_unlock()
{
    ...
    BaseType_t status = xSemaphoreGive( xMlMutex );
    if (status != pdPASS) {
...
static void ARM_SAI_SignalEvent(uint32_t event)
{
    ...
    if ((event & TX_UNDERFLOW_Msk) == ARM_SAI_EVENT_TX_UNDERFLOW) {
        LogError( ( "Error TX is enabled but no data is being sent\n" ) );
    }
    if ((event & RX_OVERFLOW_Msk) == ARM_SAI_EVENT_RX_OVERFLOW) {
        LogError( ( "Error RX is enabled but no data is being received\n" ) );
    }
    if ((event & FRAME_ERROR_Msk) == ARM_SAI_EVENT_FRAME_ERROR) {
        LogError( ( "Frame error occured\n" ) );
    }
}
...
template <typename T> struct CircularSlidingWindow {
                                                                                                          ...
                                                                                                          stride_size}
    {
        ...
        create_binary_semaphore(&xSlidingWindowSemaphore);
    }
    ...
    static void create_binary_semaphore(SemaphoreHandle_t* xSemaphore)
    {
...
void ProcessAudio(ApplicationContext &ctx)
{
    ...
    while (true) {
        ...
        while (true) {
            EventBits_t flags = xEventGroupWaitBits(xAppEvents, (EventBits_t)ML_EVENT_STOP, pdTRUE, pdFAIL, 0);
            if ((flags & ML_EVENT_STOP) == 0U)
            {
                flags = (uint32_t)ERROR_RESOURCE;
            }
        ...
        while (true) {
            EventBits_t flags = xEventGroupWaitBits(xAppEvents, (EventBits_t)ML_EVENT_START, pdTRUE, pdFAIL, portMAX_DELAY);
            if ((flags & ML_EVENT_START) == 0U)
            {
                flags = (uint32_t)ERROR_TIMEOUT;
            }
...
void ml_mqtt_task(void *arg)
{
    ...
    while (1) {
        ...
        if (xQueueReceive (xMlMqttQueue, &msg, portMAX_DELAY) == pdPASS) {
            mqtt_send_message(get_inference_result_string(msg.state));
        } else {
