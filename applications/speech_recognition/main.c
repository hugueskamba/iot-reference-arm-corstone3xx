...
QueueHandle_t xDspMsgQueue = NULL;
typedef struct
{
    char * result;
} ml_mqtt_msg_t;
...
int main( void )
{
    ...
    xDspMsgQueue = xQueueCreate( 10, sizeof( dsp_msg_t ) );

    if( xDspMsgQueue == NULL )
    {
        LogError( ( "Failed to create xDspMsgQueue\r\n" ) );
        return EXIT_FAILURE;
    }
    ...
    void * dspMLConnection = dsp_get_ml_connection();

    vStartMlTask( dspMLConnection );

    vStartDSPTask( dspMLConnection );
