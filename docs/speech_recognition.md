...
`build/applications/speech_recognition/speech-recognition-update_signed.bin` for the
speech-recognition application. The updated binary is already signed and it is the
...
1. Select upload new file and select the signed update binary
   (`build/applications/speech_recognition/speech-recognition-update_signed.bin`)
...
1. Paste the signature string that is echoed during the build of the example
   ...
   `build/applications/speech_recognition/update-signature.txt`).
...
```bash
./tools/scripts/build.sh speech-recognition
```
