...
```bash
./tools/scripts/build.sh keyword-detection --certificate_path <certificate pem's path> --private_key_path <private key pem's path>
```
...
`build/applications/keyword_detection/keyword-detection-update_signed.bin` for the
...
1. Select upload new file and select the signed update binary
   (`build/applications/keyword_detection/keyword-detection-update_signed.bin`)
...
1. Paste the signature string that is echoed during the build of the example
   ...
   `build/applications/keyword_detection/update-signature.txt`).
...
```bash
./tools/scripts/build.sh keyword-detection
```
